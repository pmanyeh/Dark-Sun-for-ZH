...
- **最新版本**：`scratch_test/cjk_display_staging_v129_class_diamond`（2026-09-26）。
  - v129 = v127（創角畫面，已實機測試）＋ NAME 列上移 3px ＋ 職業清單 ◆ 對齊。後兩項**還沒實機看過**，下次開遊戲先確認（見 1.1）。
...
- v129 的創角畫面：職業清單的 ◆ 是否對齊中文字、NAME 列上移後是否正常。
- 戰鬥中的 `Z`／`X`／`D`（v121 起的遊標熱鍵，還沒在戰鬥中測過）。

### 1.2 WIND／ICON 按鈕（學法術卷軸的 EXIT、背包的 DROP／SPLIT 等）
