...
- **最新版本**：`scratch_test/cjk_display_staging_v121_original_keys`（2026-09-26），已實機測試（見 re_104 §18）。
...
### 1.1 v121：原版按鍵還原、遊標熱鍵改為 Z／X／D（已完成，re_104 §18）

- 原本規劃的組合鍵方案（Ctrl+S、Ctrl+T、Alt+F6）已經放棄，原因見第 0 節的專案原則。
- 已實機確認：`Z`／`X`／`D`、`A` 動畫開關、空白鍵；除錯模式的 `S`（金錢）、`T`（全隊經驗值）、F6（格線）、
  按住 Shift 穿牆／遠距離觀察。還沒測：戰鬥中的 `Z`／`X`／`D`，下次遊玩時順便確認。
- 做法摘要：
  - 地圖按鍵處理程式的查表迴圈改寫成 `repne scasw`，空出 12 bytes 放兩個 marker。
  - 查不到的鍵交給 FONT 核心（`BEF2`），核心只接 `Z X D`，其他鍵照原版處理。
  - 懸停掛鉤改成就地修補 `0x1D86C`，不再借用 F6 的死碼。
  - 觀察路徑的兩個除錯分支由核心照原版邏輯執行。
- v121 的原版按鍵程式碼已經和 Steam 版逐 byte 比對，完全相同。
