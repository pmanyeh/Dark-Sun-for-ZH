...
- **最新版本**：`scratch_test/cjk_display_staging_v120_scroll_update_rect`（2026-09-25）。
  - 操作改良到 v117 為止，使用者都已實機確認。
  - v118～v120 修正學法術卷軸下方的兩行文字（見下方「2026-09-25 這一輪總結」），使用者已實機確認 v120。
- **2026-09-25 這一輪總結**（詳見 re_104）：
  1. **操作改良 v107～v117**：遊標模式熱鍵、智慧遊標、玩家用說明（下方條列）。
     - commit：`022de66`、`854a073`、`724b8d9`、`a22e669`。
  2. **學法術卷軸 v118～v120**（re_104 §16）：
     - v118：卷軸下方兩行（「選擇法術，」＋角色名稱、「學習」＋法術名稱、法術名稱＋「已學會」）原本走不會解碼的 `339E:016D`。
       四個繪製點改成 tag `FF89` redirect，在畫的那一刻才解碼。
     - v119：中文兩行原本重疊，改成第一行 y−3、第二行 y+1。
     - v120：螢幕只更新 y+2～y+16，所以上方殘留舊字的上半部。更新範圍改成 y−3～y+18。
  3. **存檔複製規則**：新版要從使用者最後玩的那一版複製全部 `SAVE*.SAV`、`DARKRUN.GFF`、`CHARSAVE.GFF`（見 0.2）。
  4. **延後處理**：卷軸左下角「第?級」的顏色偏淡。使用者說應該是深色，決定之後再處理（re_104 §16）。
- **下一個 Session 的第一個目標（使用者指定）：調整除錯功能的按鍵衝突**（re_104 §17）。
  - 原版有開發者除錯模式，以命令列 `-k911` 開啟（`[11B0]=1`）。
  - 這一輪的操作改良佔用了除錯模式的幾個入口：
  ...
  - 要先和使用者討論：除錯功能要保留哪些、改到哪些鍵，以及 stub 需要搬到哪裡。
    - 例如除錯模式開啟時，`S`／`T` 是否要恢復原本的除錯作用。
    - 常駐段和 overlay 都沒有現成的空地，這次用的都是除錯死碼。
...
- **最新 commit**：見 `git log`。到 v120 為止都已 commit 並 push（2026-09-25）。
...
4. **學法術卷軸**：
   - 下方兩行：v118～v120 已處理（re_104 §16），使用者已實機確認 v120。
   - 左下角「第?級」顏色偏淡，使用者說應該是深色，**之後再處理**。目前的線索：
...
### 0.2 重建 v120（`scratch_test/` 在 `.gitignore` 裡）
...
- 建好後，從**使用者最後玩的那一版**複製**全部**存檔：`SAVE*.SAV`（使用者會存到第 10 格以上）、`DARKRUN.GFF`、`CHARSAVE.GFF`。
